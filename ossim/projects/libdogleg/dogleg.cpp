...
#pragma comment(lib, "libdogleg.lib")
                         ...
                         dogleg_callback_t* f, void* cookie)
{
  ...
  for(i=0; i<Nmeas; i++)
  {
    ...
    if(std::numeric_limits<double>::quiet_NaN() == gRep)
    {
      if( gObs != 0 )
...
static void computeCauchyUpdate(dogleg_operatingPoint_t* point)
{
  ...
  norm2_Jt_x       = norm2(point->Jt_x, point->Jt->nrow);
  norm2_J_Jt_x     = norm2_mul_spmatrix_t_densevector(point->Jt, point->Jt_x);
  k                = -norm2_Jt_x / norm2_J_Jt_x;
...
static void computeGaussNewtonUpdate(dogleg_operatingPoint_t* point, dogleg_solverContext_t* ctx)
{
  ...
  Jt_x_dense.nrow = point->Jt->nrow;
  Jt_x_dense.ncol  = 1;
  Jt_x_dense.nzmax = point->Jt->nrow;
  Jt_x_dense.d     = point->Jt->nrow;
  Jt_x_dense.x     = point->Jt_x;
  Jt_x_dense.xtype = CHOLMOD_REAL;
  Jt_x_dense.dtype = CHOLMOD_DOUBLE;
  ...
  point->updateGN_lensq = norm2((double*)point->updateGN_cholmoddense->x,
                                point->Jt->nrow);
  if( DOGLEG_DEBUG )
                                      ...
                                      double                   trustregion)
{
  ...
  // +sqrt side, since the other one is negative
	...
	const double* a;
	const double* b;
	double* a_minus_b = new double[point->Jt->nrow];

  dsq    = trustregion*trustregion;
  norm2a = point->updateCauchy_lensq;
  a      = point->updateCauchy;
  b      = (double*)point->updateGN_cholmoddense->x;
                           ...
                           double trustregion, dogleg_solverContext_t* ctx)
{
  ...
  double *update_array = new double[pointFrom->Jt->nrow];
  double* update;
  ...
  else
  {
    ...
    if(pointFrom->updateGN_lensq <= trustregion*trustregion)
    {
      ...
      update = (double*)pointFrom->updateGN_cholmoddense->x;
...
{
  dogleg_operatingPoint_t* point = new dogleg_operatingPoint_t[sizeof(dogleg_operatingPoint_t)];
  ASSERT(point != NULL);

  point->p = new double[Nstate];
  ASSERT(point->p != NULL);

  point->x = new double[numMeasurements];
  ASSERT(point->x != NULL);
  ...
  point->Jt_x = new double[Nstate];
  ASSERT(point->Jt_x != NULL);

  // the cached update vectors
  point->updateCauchy          = new double[Nstate];
...
{
  dogleg_solverContext_t* ctx = new dogleg_solverContext_t;
  ctx->f                      = f;
  ctx->cookie                 = cookie;
