...
#include <ossim/projection/ossimHj1Model.h>
#include <ossim/projection/ossimTheosModel.h>
...
#include <ossim/support_data/ossimTheosDimapSupportData.h>
   ...
   const ossimFilename& filename, ossim_uint32  entryIdx) const
{
   ...
   if(spot5Test.exists())
   {
	  ...
	  fstream inf;
	  inf.open(spot5Test.c_str(), ios_base::in);
	  int i = 0;
	  while(i < 10)
	  {
		  string strtmp;
		  ...
		  if(str.upcase().contains("SPOTSCENE_1A"))
		  {
			  // Spot
   ...
   ossimFilename theosTest = geomFile;
   if(!theosTest.exists())
   {
   ...
   if(theosTest.exists())
   {
	   ossimRefPtr<ossimTheosDimapSupportData> meta =
		   new ossimTheosDimapSupportData;
	   fstream inf;
	   inf.open(theosTest.c_str(), ios_base::in);
	   int i = 0;
	   while(i < 10)
	   {
		   string strtmp;
		   ...
		   if(str.upcase().contains("THEOS1_1A_SCENE"))
		   {
			   ...
			   {
				   model = new ossimTheosModel(meta.get());
				   if(!model->getErrorStatus())
				   {
					   inf.close();
					   return model.release();
   ...
   if(Hj1Test.exists())
   {
      ...
      {
         model = new ossimHj1Model(meta.get());
         if(!model->getErrorStatus())
         {
            return model.release();
         }
