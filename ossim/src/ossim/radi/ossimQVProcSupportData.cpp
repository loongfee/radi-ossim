...
{	
	...
	while(os.getline(strtmp,2048))
	{
		...
		theSampTimes.push_back(strList[0].toDouble());
                                               ...
                                               ossimEcefPoint& pe)  const
{
   ...
   if ((time <  getSampTime(0)) ||
       (time >= getSampTime(theSampTimes.size()-1)))
   {
	   ossimDpt3d tempPt;
	   int last_samp = (int) theSampTimes.size() - 1;
	   if (last_samp < 1)
		   return;
	   ...
	   if (time < getSampTime(0))
	   {
		   double dt = (time - getSampTime(0)) * line_time;
		   tempPt = thePosEcfSamples[0] + (theVelEcfSamples[0]*dt);
	   }
	   else if (time >= getSampTime(theSampTimes.size()-1))
	   {
		   double dt = (time - getSampTime(last_samp)) * line_time;
   ...
   //***
   int i=0;
   while ((i < (int)theSampTimes.size()) &&
          (getSampTime(i) < time)) ++i;
   --i;
   ossim_float64 dt1   = time - getSampTime(i);
   ossim_float64 dt0   = getSampTime(i+1) - time;
   ossim_float64 dt    = getSampTime(i+1) - getSampTime(i);
...
void ossimQVProcSupportData::extrapolateAttitude(const ossim_float64& time, ossimDpt3d& at) const
{
   ...
   //   delta_t = time - theAttSampTimes[last_samp];
	  ...
	  ////at = (theAttitudeSamples[i]+theAttitudeBias[i])*(1.0+2.0*dtt1/dtt)*dtt0*dtt0/(dtt*dtt)
		 ...
		 //// + theAttitudeVelSamples[i]*dtt1*dtt0*dtt0/(dtt*dtt)
	  //// - theAttitudeVelSamples[i+1]*dtt0*dtt1*dtt1/(dtt*dtt);
	  ossim_float64 dt = (time - getSampTime(last_samp)) * line_time;
...
{
   return (ossim_uint32)theSampTimes.size();
}
                                          ...
                                          const char* prefix)const
{
   ...
   tempString = "";
   for(idx = 0; idx < theSampTimes.size(); ++idx)
   {
      tempString += (ossimString::toString(theSampTimes[idx]) + " ");
   }
   kwl.add(prefix,
           "sample_times",
           tempString,
           true);
   kwl.add(prefix,
           "number_of_sample_times",
           static_cast<ossim_uint32>(theSampTimes.size()),
           true);
                                          ...
                                          const char* prefix)
{
   ...
   total =  ossimString(kwl.find(prefix,"number_of_sample_times")).toUInt32();
   theSampTimes.resize(total);
   tempString = kwl.find(prefix,"sample_times");
   if(tempString != "")
   {
      std::istringstream in(tempString.string());
      ossimString tempValue;
      ...
      {
         in >> tempValue.string();
         theSampTimes[idx] = tempValue.toDouble();
      }
   }
