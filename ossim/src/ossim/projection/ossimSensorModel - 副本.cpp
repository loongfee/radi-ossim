...
/*Robust专区**Robust专区**Robust专区**Robust专区**Robust专区**Robust专区**Robust专区**Robust专区**Robust专区**Robust专区**Robust专区*/
	...
	while ( (!found) && (iter < iter_max) ) //non linear optimization loop
	{
		...
		do
		{
			...
			else 
			{
				...
				if (uk>10) uk =10; 
				//DEBUG TBR
				cout<<"均方差"<<sqrt( newki2power/nobs )<<" "<<endl;
				cout.flush();
				if (res_reduction > 0)
				{
					cparm = nparm;
		...
		do
		{
			...
			else 
			{
				...
				if (res_reduction > 0)
				{
					...
					deltap_scale = max(1e-15, deltap.NormInfinity()*1e-4);
					buildNormalEquation(tieSet, A, residue, projResidue, deltap_scale);
					cout<<"_____________________循环中residue____________________________"<<endl;
					for( int i=1; i<=nobs;i++)
					{
						cout<<i<<"  "<<residue(3*i-2)<<" "<< residue(3*i-1)<<" "<< residue(3*i)<<endl;
					}
					cout<<"reduction时迭代计算的A的对角线值"<<endl;
					for (int i=1; i<=np;i++)
					{
						cout<<A(i,i)<<endl;
					}
...
NEWMAT::DiagonalMatrix ossimSensorModel::newobvPower( NEWMAT::ColumnVector &thelastresidue, NEWMAT::DiagonalMatrix obvPower, int robustmode) const
{
	...
	case TURKEY:
		//
		cout<<"使用Turkey法,请输入turkeyc"<<endl;
		double turkeyc;
		cin>>turkeyc;
	...
	case HAMPEL:
		//选权迭代采用Hampel法
		//常数的选取《抗差估计中几种选权迭代法常数选取的探讨》	
		//double Hampela, Hampelb, Hampelc;
		//cout<<"使用Hampel法,请输入Hampela"<<endl;
		//cin>>Hampela;
		//cin>>Hampelb;
		//cin>>Hampelc;
		//Hampela=2;
	...
	case Anddrews:
		//
		cout<<"使用Anddrews法,请输入Anddrewsc"<<endl;
		double Anddrewsc;
		cin>>Anddrewsc;
	...
	case DENMARK:
		//
		cout<<"使用Denmark法,,请输入Denmarkk"<<endl;
		double Denmarkk;
		cin>>Denmarkk;
...
NEWMAT::DiagonalMatrix ossimSensorModel::newobvPowerMedian( NEWMAT::ColumnVector &thelastresidue, NEWMAT::DiagonalMatrix obvPower) const
{
	...
	//冒泡排序绝对离差中位数,X Y方向，注意与前面求中位数的区别：这个中位数为正数
