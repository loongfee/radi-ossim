									   ...
									   const std::vector<double>& z)const
{
	...
	int nCols = m.Ncols();
	NEWMAT::IdentityMatrix lambda(nCols);
	lambda = lambda * 1.0e-6;
	coeff = invert(m.t()*m+lambda)*m.t()*r;
...
void ossimRpcSolver::update_normalization_params(const std::vector<ossimDpt>& imagePoints,
								   const std::vector<ossimGpt>& groundControlPoints)
{
	// compute the image bounds for the given image points
	//
...
void ossimRpcSolver::normalize(ossimGpt &gpt)
{
...
void ossimRpcSolver::denormalize(ossimDpt &dpt)
{
