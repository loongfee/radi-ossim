                                            ...
                                            bool processSwir)
{
   ...
   FILE* pf;
   pf = fopen("att.txt", "w+");
   for (int i = 0; i < (int)theAttitudeSamples.size(); i += 1)
   {
	   if (i > 0)
	   {
		   fprintf(pf, "\n");
	   }
	   ossimDpt3d at = theAttitudeSamples[i];
	   double time = theAttSampTimes[i];
	   int nLine = int((theAttSampTimes[i] - theRefLineTime) / theLineSamplingPeriod + theRefLineTimeLine + 0.5);
	   //meta->getAttitude(ossim_uint32(i), at);
	   fprintf(pf, "%5d%20.15lf%20.15lf%20.15lf", nLine, at.x, at.y, at.z);
   }
   fclose(pf);
...
void ossimSpotDimapSupportData::getPositionEcf(const long double& time_offset,
											   ossimEcefPoint& pe)  const
{
	//long double time = theRefLineTime + time_offset;
	...
	//	if (time < theEphSampTimes.front())
	//	{
	//		double dt = (theRefLineTime - theEphSampTimes.front() + time_offset);
	//		tempPt = thePosEcfSamples[0] + (theVelEcfSamples[0] * dt);
	//	}
	//	else if (time >= theEphSampTimes.back())
	//	{
	//		double dt = (theRefLineTime - theEphSampTimes.back() + time_offset);
	...
	//--i;
	//long double dt1 = theRefLineTime - theEphSampTimes[i] + time_offset;
	//long double dt0 = theEphSampTimes[i + 1] - theRefLineTime - time_offset;
	//long double dt = theEphSampTimes[i + 1] - theEphSampTimes[i];
	//long double dtt1 = dt1;
	//long double dtt0 = dt0;
	//long double dtt = dt;
	...
	if((thePosEcfSamples.size() < 8)||
		(theEphSampTimes.size() < 8))
	{
		getBilinearInterpolation(time_offset, thePosEcfSamples, theEphSampTimes, tempPt);
	}
	else
	{
		getLagrangeInterpolation(time_offset, thePosEcfSamples, theEphSampTimes, tempPt);
	}
...
//	   if (time < theEphSampTimes.front())
//	   {
//		   double dt = (time - theEphSampTimes[0]);
//		   tempPt = thePosEcfSamples[0] + (theVelEcfSamples[0]*dt);
//	   }
//	   else if (time >= theEphSampTimes.back())
//	   {
...
//
//
//   ossimDpt3d tempPt = thePosEcfSamples[i]*(1.0+2.0*dt1/dt)*dt0*dt0/(dt*dt)
//	   + thePosEcfSamples[i+1]*(1.0+2.0*dt0/dt)*dt1*dt1/(dt*dt)
//	   + theVelEcfSamples[i]*dt1*dt0*dt0/(dt*dt)
//	   - theVelEcfSamples[i+1]*dt0*dt1*dt1/(dt*dt);
//
//   pe = ossimEcefPoint(tempPt.x,
//                       tempPt.y,
//                       tempPt.z);
...
void ossimSpotDimapSupportData::getVelocityEcf(const long double& time_offset,
                                               ossimEcefPoint& ve)  const
{
	...
	//		double delta_t = theRefLineTime - theEphSampTimes[0] + time_offset;
	...
	//		ossimDpt3d dAtt_dt = dAtt / dt;
	//		double delta_t = theRefLineTime - theEphSampTimes[last_samp] + time_offset;
   ...
   if((theVelEcfSamples.size() < 8) ||
      (theEphSampTimes.size() < 8))
   {
	   getBilinearInterpolation(time_offset, theVelEcfSamples, theEphSampTimes, tempPt);
   }
   else
   {
	   getLagrangeInterpolation(time_offset, theVelEcfSamples, theEphSampTimes, tempPt);
   }
...
//   //tempPt = (theVelEcfSamples[i+1]*dt1 + theVelEcfSamples[i]*dt0)/dt;
//
//   tempPt = theVelEcfSamples[i]*(1.0+2.0*dt1/dt)*dt0*dt0/(dt*dt)
//	   + theVelEcfSamples[i+1]*(1.0+2.0*dt0/dt)*dt1*dt1/(dt*dt);
//
//
//   ve = ossimEcefPoint(tempPt.x,
//                       tempPt.y,
...
void ossimSpotDimapSupportData::getAttitude(const long double& time_offset,
                                            ossimDpt3d& at)  const
{
	ossimDpt3d tempPt;

	if ((theAttitudeSamples.size() < 8) ||
		(theAttSampTimes.size() < 8))
	{
		getBilinearInterpolation(time_offset, theAttitudeSamples, theAttSampTimes, tempPt);
	}
	else
	{
		getLagrangeInterpolation(time_offset, theAttitudeSamples, theAttSampTimes, tempPt);
	}
   ...
   {
	   extrapolateAttitude(time_offset, at);
      return;
   }
   ...
   //***
   long double dt1 = theRefLineTime - theAttSampTimes[i] + time_offset;
   long double dt0 = theAttSampTimes[i + 1] - theRefLineTime - time_offset;
   long double dt = theAttSampTimes[i + 1] - theAttSampTimes[i];

   //dt1 = floor(dt1 * 1.0e8 + 0.5) * 1.0e-8;
   //dt0 = floor(dt0 * 1.0e8 + 0.5) * 1.0e-8;
   //dt = floor(dt * 1.0e8 + 0.5) * 1.0e-8;

   at = (theAttitudeSamples[i+1]*dt1 + theAttitudeSamples[i]*dt0)/dt;
}

void ossimSpotDimapSupportData::extrapolateAttitude(const long double& time_offset, ossimDpt3d& at) const
{
	long double time = theRefLineTime + time_offset;
   at.makeNan();
   ...
   if (time < theAttSampTimes.front())
   {
      ...
      dAtt_dt = dAtt/dt;
	  delta_t = theRefLineTime - theAttSampTimes[0] + time_offset;
   ...
   else if (time >= theAttSampTimes.back())
   {
      ...
      dAtt_dt = dAtt/dt;
	  delta_t = theRefLineTime - theAttSampTimes[last_samp] + time_offset;
...
//              << "T size = " << T.size() << std::endl;
   ...
   for (ossim_uint32 j=samp0; j<(samp0+filter_size+bump); ++j)
   {
      ...
      for (ossim_uint32 i=samp0; i<(samp0+filter_size+bump); ++i)
      {
		 ...
		 {
			//numerator   *= time - T[i];
			//numerator *= floor((time - T[i])*1.0e8 + 0.5)*1.0e-8;
   //         denominator *= T[j] - T[i];
			 long double a1 = theRefLineTime - T[i] + time_offset;
			 long double a2 = T[j] - T[i];
			coeff *= a1 / a2;
         }
      }
...
void ossimSpotDimapSupportData::getRefLineTime(long double& rt) const
{
   rt = theRefLineTime;
}
                                          ...
                                          const char* prefix)const
{
   ...
   {
      tempString += (ossimString::toString((double)theAttSampTimes[idx]) + " ");
   }
   kwl.add(prefix,
   ...
   {
      tempString += (ossimString::toString((double)theEphSampTimes[idx]) + " ");
   }
