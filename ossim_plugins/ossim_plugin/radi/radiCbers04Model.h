#ifndef radiCbers04Model_HEADER
#define radiCbers04Model_HEADER
#include <iostream>
using namespace std;
...
#include <ossim_plugin/radi/radiCbers04SupportData.h>
...
class OSSIM_PLUGINS_DLL radiCbers04Model : public ossimSensorModel
{
public:
   /*!
    * CONSTRUCTORS:
    */
	radiCbers04Model();
	radiCbers04Model(ossimQVProcSupportData* sd);
	radiCbers04Model(radiCbers04SupportData* sd);
	radiCbers04Model(const ossimFilename& init_file);
	radiCbers04Model(const ossimKeywordlist& geom_kwl);
	radiCbers04Model(const radiCbers04Model& rhs);
   virtual ~radiCbers04Model();
