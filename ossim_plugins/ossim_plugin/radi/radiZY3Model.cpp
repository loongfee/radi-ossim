...
// Description:
//
// Contains definition of class radiZY3Model.
//
//*****************************************************************************
// $Id: radiZY3Model.cpp 19658 2011-05-26 13:16:06Z gpotts $
...
{
	static ossimTrace traceExec  ("radiZY3Model:exec");
	static ossimTrace traceDebug ("radiZY3Model:debug");
	RTTI_DEF1(radiZY3Model, "radiZY3Model", ossimSensorModel);
...
radiZY3Model::radiZY3Model(ossimQVProcSupportData* sd)
   :
   ossimSensorModel      (),
   theSupportData        (sd),
   theMetaDataFile       ("NOT ASSIGNED"),
...
//   theOrbToEcfRotation   (3, 3),
   ...
   theApproximateModel(NULL)
{
   if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model(desc_file) Constructor: entering..." << std::endl;
   ...
   {
      if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model(desc_file) Constructor: returning with error..." << std::endl;
      return;
   }
   ...
   if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model(desc_file) Constructor: returning..." << std::endl;
}

radiZY3Model::radiZY3Model(radiZY3SupportData* sd)
	:
	ossimSensorModel      (),
	theMetaDataFile       ("NOT ASSIGNED"),
	theIllumAzimuth       (0.0),
...
//*****************************************************************************
//  DESTRUCTOR: ~radiZY3Model()
//
//*****************************************************************************
radiZY3Model::~radiZY3Model()
{
   if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG DESTRUCTOR: ~radiZY3Model(): entering..." << std::endl;

   theSupportData = 0;
   
   if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG DESTRUCTOR: ~radiZY3Model(): returning..." << std::endl;
}

radiZY3Model::radiZY3Model(const radiZY3Model& rhs)
...
void radiZY3Model::getCcdToSatRotation(NEWMAT::Matrix& result)const
{
	result = NEWMAT::Matrix(3,3);
	double roll, pitch, yaw;
	if (theSensorID.upcase().contains("FWD"))
	{
	...
	else if(theSensorID.upcase().contains("NAD"))
	{
		roll = 0.0;
		pitch = 0.0;
		yaw = 0.0;
	}
	else if(theSensorID.upcase().contains("BWD"))
	{
...
void radiZY3Model::computeSatToOrbRotation(NEWMAT::Matrix& result, ossim_float64 t)const
{
   if (traceExec())
   {
      ossimNotify(ossimNotifyLevel_DEBUG)
      << "DEBUG radiZY3Model::computeSatToOrbRotation(): entering..."
      << std::endl;
   }
   //---
   ...
   if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model::computeSatToOrbRotation(): returning..." << std::endl;
}
...
//  METHOD
//*****************************************************************************
void radiZY3Model::computeSatToOrbRotation(ossim_float64 t)const
{
   if (traceExec())
   {
      ossimNotify(ossimNotifyLevel_DEBUG)
...
ossim_float64 radiZY3Model::computeLookAngleY(double samp, double ccdOffset/* = 0.0*/) const
{
...
void radiZY3Model::computePhiXY(double samp, double &phiX, double& phiY) const
{
	double ps = 0.000065;
...
ossim_float64 radiZY3Model::computePhiX(double samp) const
{
	double ps = 0.000065;
	double midCCD = theImageSize.samp * 0.5;
...
ossim_float64 radiZY3Model::computePhiY(double samp) const
{
	double ps = 0.000065;
	if (theSensorID.upcase().contains("FWD"))
	{
...
void radiZY3Model::create3DGridPoints(const ossimDrect& imageBounds,
									   double height,
...
void radiZY3Model::updateApproximateModel()
{
...
//*****************************************************************************
// PUBLIC METHOD: radiZY3Model::updateModel()
//
//  Updates the model parameters given the normalized adjustable parameter
//  array.
//
//*****************************************************************************
void radiZY3Model::updateModel()
{
   clearErrorStatus();

   try
   {
      if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model::updateModel(): entering..." << std::endl;
      ...
      if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model::updateModel(): returning..." << std::endl;
   }
   catch(...)
   {
...
void radiZY3Model::initAdjustableParameters()
{
   if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model::initAdjustableParameters(): entering..." << std::endl;
   ...
   if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model::initAdjustableParameters(): returning..." << std::endl;
}

void radiZY3Model::loadSupportData()
{
   if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "radiZY3Model::loadSupportData(): entering..." << std::endl;
   ...
   {
      setErrorStatus();
      ossimNotify(ossimNotifyLevel_FATAL) << "FATAL radiZY3Model::loadSupportData(): Null SpotDimapSupportData pointer passed to"
                                          << " constructor! Aborting..." << std::endl;
      if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model::loadSupportData(): returning..." << std::endl;
      return;
   }
   ...
   {
      setErrorStatus();
      ossimNotify(ossimNotifyLevel_FATAL) << "FATAL radiZY3Model::loadSupportData(): Bad SpotDimapSupportData detected. Aborting..."
                                          << std::endl;
      if (traceExec()) ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model::loadSupportData(): returning..." << std::endl;
      return;
   }
...
ossimObject* radiZY3Model::dup() const
{
   return new radiZY3Model(*this);
}

std::ostream& radiZY3Model::print(std::ostream& out) const
{
   // Capture stream flags since we are going to mess with them.
   std::ios_base::fmtflags f = out.flags();

   out << "\nDump of radiZY3Model at address " << (hex) << this
       << (dec)
       << "\n------------------------------------------------"
...
bool radiZY3Model::saveState(ossimKeywordlist& kwl,
...
bool radiZY3Model::loadState(const ossimKeywordlist& kwl,
                                const char* prefix)
{
   ossimString supportPrefix = ossimString(prefix) + "support_data.";
...
void radiZY3Model::imagingRay(const ossimDpt& image_point,
							   ossimColumnVector3d& image_ray) const
{
	...
	{
		ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model::imagingRay():------------ BEGIN DEBUG PASS ---------------" << std::endl;
		ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model::imagingRay(): t_line = " << t_line << std::endl;
	}
...
double radiZY3Model::calcGAST(double t) const
{
...
void radiZY3Model::getExteriorParameters(const ossimDpt& image_point, 
...
ossimEcefPoint radiZY3Model::collinearEquation_inverse(const ossimDpt& image_point,
...
ossimDpt radiZY3Model::collinearEquation_forward(const ossimEcefPoint& ecefPoint,
											  const NEWMAT::Matrix& rotationMat,
											  const ossimEcefPoint& satPoint) const
{
	double a1 = rotationMat.element(0, 0);
...
void radiZY3Model::collinearEquation_deriv(const ossimEcefPoint& ecefPoint,
											 const NEWMAT::Matrix& rotationMat,
...
void radiZY3Model::lineSampleToWorld(const ossimDpt& image_point,
										 ossimGpt&       gpt) const
{
...
//*****************************************************************************
//  METHOD: radiZY3Model::worldToLineSample()
//  
//  Performs forward projection of ground point to image space.
//  
//*****************************************************************************
void radiZY3Model::worldToLineSample(const ossimGpt& worldPoint,
                                         ossimDpt&       ip) const
{
...
void radiZY3Model::collinearEquation_lineSampleToWorld(const ossimDpt& image_point,
...
void radiZY3Model::lineSampleHeightToWorld(const ossimDpt& image_point,
...
//   if (!insideImage(image_point))
   if ( !theImageClipRect.pointWithin(image_point, 1.0-FLT_EPSILON) )   
   {
      ...
      if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model::lineSampleHeightToWorld(): returning..." << std::endl;
      return;
   }
   //***
...
bool
radiZY3Model::setupOptimizer(const ossimString& init_file)
{
...
bool
radiZY3Model::initFromMetadata(ossimQVProcSupportData* sd)
{
   // init parms
   theSupportData        = sd;
   theMetaDataFile       = "NOT ASSIGNED";
...
//   theOrbToEcfRotation   = 0.0; //matrix
   ...
   if (getErrorStatus() != ossimErrorCodes::OSSIM_OK)
   {
      if (traceExec())  ossimNotify(ossimNotifyLevel_DEBUG) << "DEBUG radiZY3Model::initFromMetadata(dimap_file): returning with error..." << std::endl;
      return false;
   }
...
void radiZY3Model::setImageRect(const ossimDrect& roi)
{
...
//ossimDpt
//	radiZY3Model::getForwardDeriv(int iAtt, int iComponent, const ossimGpt& gpos, double hdelta)
//{   
...
//
//void
//	radiZY3Model::buildNormalEquation(const ossimTieGptSet& tieSet,
...
//
//double
//	radiZY3Model::optimizeFit(const ossimTieGptSet& tieSet, double* /* targetVariance */)
//{
