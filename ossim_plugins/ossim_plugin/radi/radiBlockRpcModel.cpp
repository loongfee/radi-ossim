#include <radi/radiBlockRpcModel.h>
...
RTTI_DEF1(radiBlockRpcModel, "radiBlockRpcModel", ossimSensorModel);

//***
// Define Trace flags for use within this file:
//***
static ossimTrace traceExec  ("radiBlockRpcModel:exec");
static ossimTrace traceDebug ("radiBlockRpcModel:debug");

static const int    MODEL_VERSION_NUMBER  = 1;
static const int    NUM_COEFFS        = 20;
static const char*  MODEL_TYPE        = "radiBlockRpcModel";
...
//  
//*****************************************************************************
radiBlockRpcModel::radiBlockRpcModel(int nSampleBlockSize, int nLineBlockSize,
									 int nSampleBlocks/* = 1*/, int nLineBlocks/* = 1*/, 
									 int nSampleOverlap/* = 500*/, int nLineOverlap/* = 500*/)
   :  ossimSensorModel(),
      thePolyType     (A),
      theLineScale    (0.0),
      theSampScale    (0.0),
      ...
      theBiasError    (0.0),
	  theRandError    (0.0),
	  m_nSampleBlockSize(nSampleBlockSize),
	  m_nLineBlockSize(nLineBlockSize),
	  m_nSampleBlocks(nSampleBlocks),
	  m_nLineBlocks(nLineBlocks),
	  m_nSampleOverlaps(nSampleOverlap),
	  m_nLineOverlaps(nLineOverlap),
	  m_rpcModelList(),
	  m_blockRectList()
{
   initAdjustableParameters();

   for (int j = 0;j < m_nLineBlocks;++j)
   {
	   for (int i = 0;i < m_nSampleBlocks;++i)
	   {
		   int ul_sample = i * (m_nSampleBlockSize - nSampleOverlap);
		   int ul_line = j * (m_nLineBlockSize - nLineOverlap);
		   m_blockRectList.push_back(ossimIrect(ul_sample, ul_line, ul_sample+m_nSampleBlockSize, ul_line+m_nLineBlockSize));
		   m_rpcModelList.push_back(new radiRpcModel);
	   }
   }
}
...
//  
//*****************************************************************************
radiBlockRpcModel::radiBlockRpcModel(const radiBlockRpcModel& model)
   :
	  ...
	  m_nSampleBlockSize(model.m_nSampleBlockSize),
	  m_nLineBlockSize(model.m_nLineBlockSize),
	  m_nSampleBlocks(model.m_nSampleBlocks),
	  m_nLineBlocks(model.m_nLineBlocks),
	  m_nSampleOverlaps(model.m_nSampleOverlaps),
	  m_nLineOverlaps(model.m_nLineOverlaps),
	  m_rpcModelList(model.m_rpcModelList),
	  m_blockRectList(model.m_blockRectList)
{
   for (int i=0; i<20; ++i  )
   {
...
//  
//*****************************************************************************
radiBlockRpcModel::~radiBlockRpcModel()
{
}

void radiBlockRpcModel::setAttributes(ossim_float64 sampleOffset,
...
void radiBlockRpcModel::setMetersPerPixel(const ossimDpt& metersPerPixel)
{
   theGSD = metersPerPixel;
   theMeanGSD = (theGSD.x+theGSD.y)*.5;
}

void radiBlockRpcModel::setPositionError(const ossim_float64& biasError,
...
//  the polynomials.
//*****************************************************************************
void radiBlockRpcModel::worldToLineSample(const ossimGpt& ground_point,
...
//*****************************************************************************
void  radiBlockRpcModel::lineSampleToWorld(const ossimDpt& imagePoint,
...
//
//*****************************************************************************
void radiBlockRpcModel::imagingRay(const ossimDpt& imagePoint,
...
//
//*****************************************************************************
void radiBlockRpcModel::lineSampleHeightToWorld(const ossimDpt& image_point,
...
//  
//*****************************************************************************
double radiBlockRpcModel::polynomial(const double& P, const double& L,
...
//  
//*****************************************************************************
double radiBlockRpcModel::dPoly_dLat(const double& P, const double& L,
                                 const double& H, const double* c) const
{
   double dr;
...
//  
//*****************************************************************************
double radiBlockRpcModel::dPoly_dLon(const double& P, const double& L,
                                 const double& H, const double* c) const
{
   double dr;
...
//  
//*****************************************************************************
double radiBlockRpcModel::dPoly_dHgt(const double& P, const double& L,
                                 const double& H, const double* c) const
{
   double dr;
...
void radiBlockRpcModel::updateModel()
{
...
void radiBlockRpcModel::initAdjustableParameters()
{
...
ossimObject* radiBlockRpcModel::dup() const
{
   return new radiBlockRpcModel(*this);
}
...
//  
//*****************************************************************************
std::ostream& radiBlockRpcModel::print(std::ostream& out) const
{
...
//  
//*****************************************************************************
bool radiBlockRpcModel::saveState(ossimKeywordlist& kwl,
...
//  
//*****************************************************************************
bool radiBlockRpcModel::loadState(const ossimKeywordlist& kwl,
                              const char* prefix) 
{
   if (traceExec())
   {
...
//  
//*****************************************************************************
void radiBlockRpcModel::writeGeomTemplate(ostream& os)
{
...
bool radiBlockRpcModel::setupOptimizer(const ossimString& init_file)
{
...
//  
//*****************************************************************************
ossimDpt radiBlockRpcModel::getForwardDeriv(int derivMode,
                                        const ossimGpt& pos,
                                        double h)
{
...
double radiBlockRpcModel::getBiasError() const
{
   return theBiasError;
}

double radiBlockRpcModel::getRandError() const
{
   return theRandError;
}
...
//  
//*****************************************************************************
void radiBlockRpcModel::getRpcParameters(radiRpcModel::rpcModelStruct& model) const
{
...
void radiBlockRpcModel::saveRpcModelStruct(fstream &fs)const
{
...
void radiBlockRpcModel::saveRpcModelStruct(radiRpcModel::rpcModelStruct& rpcStruct)const
{
...
void radiBlockRpcModel::setAttributes(radiRpcModel::rpcModelStruct& model, bool computeGsdFlag)
{
...
//int radiBlockRpcModel::initiateGpt(const ossimTieGptSet& tieSet, vector< radiBlockTieGpt > &GptList)
//{
//	int nGcps = (int)tieSet.getTiePoints().size();
//	GptList.clear();
//	for (int i = 0;i < nGcps;++i)
//	{
//		vector<int> idList = getRpcModelId(tieSet.getTiePoints()[i]->getImagePoint());
//	}
//	ossimGpt gpt;
//
//	m_nUnknownPoints = 0;
...
//}s
//
//double radiBlockRpcModel::optimizeFit(const ossimTieGptSet& tieSet, double* targetVariance)
//{
//#if 1
...
ossim_uint32 radiBlockRpcModel::getNumberOfAdjustableParameters()const
{
...
int radiBlockRpcModel::findChar(ossimString str, vector<char> chList)
{
	// 获取分隔符的个数
...
void radiBlockRpcModel::splitString(ossimString str, vector<char> chList, vector<ossimString>& strArray)
{
	strArray.clear();
...
bool radiBlockRpcModel::readRPBFile(ossimFilename rpcFile, ossimplugins::radiRpcModel::rpcModelStruct& rpcStruct)
{
...
bool radiBlockRpcModel::readRPCFile(ossimFilename rpcFile, ossimplugins::radiRpcModel::rpcModelStruct& rpcStruct)
{
	std::fstream fs;
...
bool radiBlockRpcModel::parseRpcFile(ossimFilename imageFilename)
{
...
int radiBlockRpcModel::BlockId2Idx(const BlockId& block_id)const
{
	return m_nSampleBlocks * block_id.iLineBlock + block_id.iSampleBlock;
}

vector<int> radiBlockRpcModel::getRpcModelId(const ossimDpt& dpt)const
{
	vector<int> idList;
	for (int i = 0;i < (int)m_rpcModelList.size();++i)
	{
		if (m_blockRectList[i].pointWithin(dpt))
		{
			idList.push_back(i);
		}
	}
	if (idList.size() < 1)
	{
		cout<<"warning: point "<<dpt<<" out of range"<<endl;
		idList.push_back(0);
	}
	return idList;
}

void radiBlockRpcModel::generateTiePoints(ossim_uint32 xSamples/* = 20*/, ossim_uint32 ySamples/* = 20*/)
{
	ossim_uint32 x,y;
	ossim_float64 w = m_nSampleBlocks*(m_nSampleBlockSize-m_nSampleOverlaps) + m_nSampleOverlaps;
	ossim_float64 h = m_nLineBlocks*(m_nLineBlockSize-m_nLineOverlaps) + m_nLineOverlaps;
	ossimGpt gpt;
	...
	for(y = 0; y < ySamples; ++y)
	{
		for(x = 0; x < xSamples; ++x)
		{
			...
			ossimDpt dpt((w-1)*xnorm, (h-1)*ynorm);

			vector<int> idList = getRpcModelId(dpt);

			if (idList.size() > 1)
			{
				radiBlockTieGpt tp;
				tp.m_nType = ossimplugins::radiBlockTieGpt::unknown_tie_image_points;
				for (int iid = 0;iid < (int)idList.size();++iid)
				{
					tp.m_DptList.push_back(pair<int, ossimDpt>(iid, dpt));
				}
...
void radiBlockRpcModel::writeRpcFile(ossimFilename rpcFilename)
{
	fstream ofs;
