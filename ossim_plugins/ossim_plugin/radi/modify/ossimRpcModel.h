...
public:
   ...
   void setAttributes(ossimRpcModel::rpcModelStruct& model, bool computeGsdFlag=true);
   ...
   void saveRpcModelStruct(ossimRpcModel::rpcModelStruct& rpcStruct)const;
...
struct rpcOptimizeStruct
{
	const ossimRpcModel* pModel;
	double x;
	double y;
	double height;
};

struct rpc_lmder_functor : Functor<double>
{
public:
	rpc_lmder_functor(int np, int nobs): Functor<double>(np,nobs) {}
	int operator()(const VectorXd &x, VectorXd &fvec) const;
	int df(const VectorXd &x, MatrixXd &fjac) const;
	rpcOptimizeStruct *pData;
};
#endif
