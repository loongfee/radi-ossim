#ifndef radiBlockRpcModel_HEADER
#define radiBlockRpcModel_HEADER
...
/*!****************************************************************************
 *
 * CLASS:  radiBlockRpcModel
 *
 *****************************************************************************/
class OSSIM_PLUGINS_DLL radiBlockRpcModel : public ossimSensorModel
{
public:
   //***
   ...
   struct BlockId
   {
	   int iSampleBlock;	// begin from zero
	   int iLineBlock;	// begin from zero
   };
   ...
   radiBlockRpcModel(int nSampleBlockSize, int nLineBlockSize,
	   int nSampleBlocks = 1, int nLineBlocks = 1, 
	   int nSampleOverlap = 500, int nLineOverlap = 500);

   /** @brief copy construtor */
   radiBlockRpcModel(const radiBlockRpcModel& copy_this);

   bool parseRpcFile(ossimFilename imageFilename);

   void writeRpcFile(ossimFilename rpcFilename);
   ...
   void lineSampleHeightToWorld_Eigen(const ossimDpt& image_point,
    ...
    */
   void getRpcParameters(radiRpcModel::rpcModelStruct& model) const;
   

   int getModelParamNum()const {return 80;};

   void setAttributes(radiRpcModel::rpcModelStruct& model, bool computeGsdFlag=true);

   enum rpcOptimizeType
   {
	   OptNone,					//无改正
	   OptImageTranslation,		//平移变换
	   OptImageTrans_scale,		//平移加缩放
	   OptImageAffine,			//像方仿射变换
	   NUM_OPTIMIZE_TYPE,
   };
   void saveRpcModelStruct(fstream &fs)const;
   void saveRpcModelStruct(radiRpcModel::rpcModelStruct& rpcStruct)const;
   ...
   void lineSampleHeightToWorld2(const ossimDpt& image_point,
	   const double&   heightEllipsoid,
	   ossimGpt&       worldPoint)const;
   
   rpcOptimizeType m_modelOptimizeType;
	virtual ossim_uint32 getNumberOfAdjustableParameters()const;
...
//      YAW_OFFSET,
   ...
   virtual ~radiBlockRpcModel();

   int m_nSampleBlocks;
   int m_nLineBlocks;
   int m_nSampleBlockSize;
   int m_nLineBlockSize;
   int m_nSampleOverlaps;
   int m_nLineOverlaps;
   vector<ossimRefPtr<ossimplugins::radiRpcModel> > m_rpcModelList;
   vector<ossimIrect> m_blockRectList;
   vector<radiBlockTieGpt> m_tiePointList;
   int BlockId2Idx(const BlockId& block_id)const;	// begin from zero
   vector<int> getRpcModelId(const ossimDpt& dpt)const;
   void generateTiePoints(ossim_uint32 xSamples = 20, ossim_uint32 ySamples = 20);
   int initiateGpt(const ossimTieGptSet& tieSet, vector< radiBlockTieGpt > &GptList);
   //***
