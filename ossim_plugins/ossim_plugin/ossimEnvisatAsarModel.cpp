...
// Static trace for debugging
   ...
   bool ossimEnvisatAsarModel::InitSensorParams(const ossimKeywordlist &kwl, const char *prefix)
   {
	  ...
	  // loong
      float pixel_spacing = atof(kwl.find(prefix, "pixel_spacing"));
	  theGSD = ossimDpt(pixel_spacing, pixel_spacing);
   ...
   bool ossimEnvisatAsarModel::InitRefPoint(const ossimKeywordlist &kwl, const char *prefix)
   {
	  ...
	  // loong
      // 1: first line first pix
      // 2: first line last pix
      // 3: last line last pix
      // 4: last line first pix
	  theRefImgPt = ossimDpt((imageGCP1.x+imageGCP3.x) / 2.0, (imageGCP1.y+imageGCP3.y) / 2.0);
	  theRefGndPt = ossimGpt((groundGCP1.lat+groundGCP3.lat) / 2.0, (groundGCP1.lon+groundGCP3.lon) / 2.0, h);
